...
class OpenID_Connect_Generic_Settings_Page {
	...
	function __construct( OpenID_Connect_Generic_Option_Settings $settings, OpenID_Connect_Generic_Option_Logger $logger ) {
		...
		$fields = array(
			...
			'create_if_does_not_exist'   => array(
				...
				'description' => __( 'If the user identity is not link to an existing Wordpress user, it is created. If this setting is not enabled and if the user authenticates with an account which is not link to an existing Wordpress user then the authentication fails' ),
				'type'        => 'checkbox',
				'section'     => 'user_settings',
				'readonly'    => 1
			),
			...
			'unprotected_urls'   => array(
				'title'       => __( 'Unprotected URLs' ),
				'description' => __( 'Skip privacy for the URLs or regular expressions in the list separated by line breaks.' ),
				'type'        => 'textarea',
				'section'     => 'authorization_settings'
			),
			'protected_urls'   => array(
				'title'       => __( 'Protected URLs' ),
				'description' => __( 'Enforce privacy just for the URLs or regular expressions in the list separated by line breaks.' ),
				'type'        => 'textarea',
				'section'     => 'authorization_settings'
			),
	...
	public function sanitize_settings( $input ) {
		...
		foreach ( $this->settings_fields as $key => $field ) {
			if ( isset( $input[ $key ] ) ) {
				$options[ $key ] = $field['type'] == 'textarea' ? trim( $input[$key] ) : sanitize_text_field( trim( $input[$key] ) );
			}
			else {
				$options[ $key ] = '';
			}
	...
	public function do_textarea( $field ) {
		...
		<textarea id="<?php print esc_attr( $field['key'] ); ?>"
		       ...
		       name="<?php print esc_attr( $field['name'] ); ?>"
		       <?php if (isset($field['readonly']) && $field['readonly']) print "readonly"; ?>
			><?php echo $this->settings->{ $field['key'] }; ?></textarea>
		<?php
		$this->do_field_description( $field );
	}
